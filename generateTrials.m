...
%     % Find out how many trials per condition
%     conditions_split = strsplit(conditions, ',');
%     num_selected_conditions = length(conditions_split);
%     % Calculate the number of trials per combination
%     trials_per_condition = ceil(n_trials / num_selected_conditions);
% 
%     % Calculate the grid size in degrees of visual angle
%     grid_size_degrees = [n_rows, n_columns] .* grid_visual_angle;
%     
...
%     end
    ...
    for trial = 1:n_trials
       ...
       if mod(trial, 2) == 1
            ...
            unique_point_x = randi([2, n_rows/2 - 1]);
            target_side = 'L';
        else
            % Even trials go to the right side ('R')
            unique_point_x = randi([n_rows/2 + 1, n_rows - 1]);
            target_side = 'R';
        end
        
        % Generate a random position for the target object within the determined side of the grid
        unique_point_y = randi([2, n_columns-1]); % so its not at the corner
