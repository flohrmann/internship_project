function plotRTOverTimeColouredByCondition(data, color_map, comparison_results_folder)
    ...
    % Collect reaction times and conditions for violin plot
    for j = 1:length(condition_labels)
    ...
    violinplot(all_rt, all_conditions, 'ShowData', false, 'ShowMean', true, 'ShowBox', false);
    ...
    for j = 1:length(condition_labels)
        ...
        errorbar(x_loc, mean_rt, stderr_rt, 'k', 'LineWidth', 1.5);
    end

    % Finalize the second subplot
    title('Reaction Time Distribution per Condition with Mean and Error Bars');
    xlabel('Condition');
    ylabel('Reaction Time');
    set(gca, 'YScale', 'log');
