function ttestResults = cleanAttRTcorrTtest(merged_data, conditions)
% here ADHD/nonADHD mean high/low inattention levels according to the
% questionaire
mean_rt_adhd = [];
mean_accuracy_adhd = [];
mean_rt_nonadhd = [];
...
for i = 1:size(merged_data,1)
    ...
    if merged_data.InattentionLevel(i) == 2
        % the mean RT
    ...
    elseif merged_data.InattentionLevel(i) == 1
        % mean RT
        ...
        % inattention level
        mean_6inatt_nonadhd = [mean_6inatt_nonadhd; merged_data.PartAMeanSymptoms(i)];
...
ttestResults = runTTests_ADHD_vs_nonADHD(...
    ...
    mean_6inatt_adhd, mean_6inatt_nonadhd, ...
...
% Correlation Analysis for ADHD group
...
% % Display results for ADHD group
...
% Correlation Analysis for nonADHD group
[r_nonadhd_rt, p_nonadhd_rt]   = corr(mean_6inatt_nonadhd, [mean_rt_a_nonadhd, mean_rt_b_nonadhd, mean_rt_asimple_nonadhd, mean_rt_bsimple_nonadhd]);
[r_nonadhd_rtv, p_nonadhd_rtv] = corr(mean_6inatt_nonadhd, [rtv_a_nonadhd, rtv_b_nonadhd, rtv_asimple_nonadhd, rtv_bsimple_nonadhd]);
[r_nonadhd_con, p_nonadhd_con] = corr(mean_6inatt_nonadhd, mean_con_nonadhd);
[r_nonadhd_acc, p_nonadhd_acc] = corr(mean_6inatt_nonadhd, mean_accuracy_nonadhd);
 
% % Display results for ADHD group
for i = 1:length(r_adhd_rt)
    fprintf('%s Correlation between inattention levels and RT: r = %.2f, p = %.3f\n', conditions{i}, r_nonadhd_rt(1, i), p_nonadhd_rt(1, i));
    fprintf('%s Correlation between inattention levels and RTV: r = %.2f, p = %.3f\n', conditions{i}, r_nonadhd_rtv(1, i), p_nonadhd_rtv(1, i));
end
fprintf('Correlation between inattention levels and Accuracy: r = %.2f, p = %.3f\n', r_nonadhd_acc, p_nonadhd_acc);
fprintf('Correlation between inattention levels and Confusion: r = %.2f, p = %.3f\n', r_nonadhd_con, p_nonadhd_con);
