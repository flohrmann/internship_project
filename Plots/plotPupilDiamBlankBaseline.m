function result_table = plotPupilDiamBlankBaseline(result_table, outlier_threshold, baseline_min_length, color_map, conditions, analysis_folder)
% Extract unique conditions from result_table
num_conditions = length(conditions);

% Add a column for storing mean filtered response per condition
result_table.DiamBothBlank3StdFilter = nan(height(result_table), size(result_table.DiamBothBlank,2));
result_table.DiamBothBlank3StdFilterMean = nan(height(result_table), 1);
result_table.DiamBothBlank3StdFilterMedian =  nan(height(result_table), 1);
...
for i = 1:num_conditions
    ...
    for j = 1:size(condition_trials,1)
        ...
        if numel(current_data(~isnan(current_data))) > baseline_min_length % valid data points
            ...
            result_table.DiamBothBlank3StdFilter(trial_index_in_result_table, :) = current_data;
            result_table.DiamBothBlank3StdFilterMean(trial_index_in_result_table, :) = mean(current_data, 'omitnan');
            result_table.DiamBothBlank3StdFilterMedian(trial_index_in_result_table, :) = median(current_data, 'omitnan');
...
for i = 1:num_conditions
    ...
    for j = 1:size(condition_trials,1)
        ...
        current_data = condition_trials.DiamBothBlank3StdFilter(j,:);
        % Interpolate and normalize
        ...
        %Store processed data back in the current condition trial
        trial_index_in_result_table = condition_idx(j);
        result_table.DiamBothBlank3StdFilterZScore(trial_index_in_result_table, :) = normalized_data;
...
saveas(gcf, fullfile(analysis_folder, sprintf('blank_baseline_pupil_diam_filter.png')));
