...
classdef Titta < handle
    ...
    methods
        ...
        function setOptions(obj,settings)
            ...
            ptb_drawformattedtext_oversize = 2; % FANI both lines
        ...
        function out = init(obj, address)
            ...
            global ptb_drawformattedtext_oversize % FANI both lines
        ...
        function settings = getDefaults(tracker)
            ...
            if ~~exist('PsychtoolboxVersion','file')
                if IsWin && (~exist('libptbdrawtext_ftgl64.dll','file') || Screen('Preference','TextRenderer')==0)
                    ...
                    textFac                     = 0.75; % FANI changed from 0.75
                end
            end
            ...
            settings.UI.setup.menu.text.size        = floor(24*textFac); % FANI added FLOOR
            ...
            settings.UI.button.margins          = [14 36]; %[14 16]
            if qUsingOldWindowsPTBRenderer  % old text PTB renderer on Windows
                settings.UI.button.textVOff     = 3; % FANI no diff for buttons                           % amount (pixels) to move single line text so that it is visually centered on requested coordinate
            end
            settings.UI.button.setup.text.font              = sansFont;
            settings.UI.button.setup.text.size              = floor(24*textFac); % FANI added FLOOR
            ...
            settings.UI.button.setup.prevcal.visible        = false; %FANI was true 
            ...
            settings.UI.button.val.text.size            = floor(24*textFac); % FANI added FLOOR
            settings.UI.button.val.text.style           = 0;
            settings.UI.button.val.recal.accelerator    = 'escape';
            settings.UI.button.val.recal.visible        = false; %FANI was true 
            ...
            settings.UI.button.val.reval.visible        = false; %FANI was true 
            ...
            settings.UI.button.val.continue.visible     = false; %FANI was true 
            ...
            settings.UI.button.val.selcal.visible       = false;%FANI was true 
            ...
            settings.UI.button.val.setup.visible        = false; %FANI was true 
            ...
            settings.UI.button.val.toggGaze.visible     = false; %FANI was true 
            ...
            settings.UI.button.val.toggPlot.visible     = false; %FANI was true 
            ...
            settings.UI.plot.ax.axisLbl.size        = floor(22*textFac); % FANI added FLOOR
            ...
            settings.UI.plot.ax.tickLbl.size        = floor(18*textFac); % FANI added FLOOR
            ...
            settings.UI.plot.ax.valLbl.size         = floor(22*textFac); % FANI added FLOOR
            ...
            settings.UI.plot.but.exit.visible       = true; %FANI was true 
            ...
            settings.UI.plot.but.valSel.visible     = true; %FANI was true 
            ...
            settings.UI.val.avg.text.size       = floor(24*textFac); % FANI added FLOOR
            ...
            settings.UI.val.waitMsg.size        = floor(28*textFac); % FANI added FLOOR
            ...
            settings.UI.val.hover.text.size     = floor(20*textFac);% FANI added FLOOR
            ...
            settings.UI.val.menu.text.size      = floor(24*textFac) ; % FANI added FLOOR
            ...
            settings.UI.advcal.instruct.size    = floor(32*textFac); % FANI added FLOOR
            ...
            settings.UI.button.advcal.text.size             = floor(24*textFac);% FANI added FLOOR
            ...
            settings.UI.advcal.menu.text.size       = floor(24*textFac); % FANI added FLOOR
            ...
            settings.UI.advcal.calState.text.size   = floor(20*textFac);% FANI added FLOOR
            settings.UI.advcal.calState.text.style  = 0;
            settings.UI.advcal.avg.text.font        = monoFont;
            settings.UI.advcal.avg.text.size        = floor(24*textFac); % FANI added FLOOR
            ...
            settings.UI.advcal.hover.text.size      = floor(20*textFac); % FANI added FLOOR
            ...
            settings.UI.advcal.fixPoint.text.size   = floor(12*textFac);% FANI added FLOOR
    ...
    methods (Access = private, Hidden)
        ...
        function state = getScreenInfo(obj,wpnt)
            ...
            for w=length(wpnt):-1:1
                obj.scrInfo.resolution{w}  = Screen('Rect',wpnt(w)); 
                obj.scrInfo.resolution{w}(3:4)
                obj.scrInfo.resolution{w}(1:2) = []; % FANI 
                %obj.scrInfo.resolution{w}(1:2) = []; 
        ...
        function [status,qCalReset] = showHeadPositioning(obj,wpnt,out)            
            ...
            global ptb_drawformattedtext_oversize % FANI 
            ...
            while true
                ...
                if obj.settings.UI.setup.showInstruction && ~qHideSetup
                    ...
                    if ~isempty(str)
                        ...
                        Screen('TextSize',  wpnt(1), obj.settings.UI.setup.instruct.size);% FANI added line
        ...
        function [cache,txtbounds] = repositionTextCache(obj,cache,offset)
            if obj.usingFTGLTextRenderer
                [~,~,txtbounds,cache] = DrawFormattedText(cache,'cacheOnly',true,'sx',offset(1),'sy',offset(2)); % FANI was DrawFormattedText2
            else
        ...
        function cache = positionButtonText(obj, cache, rect, previousOff)
            ...
            % Adjust the center position of the rectangle based on scaling
            sx = sx * screenScaling;
            sy = sy * screenScaling;                  
            %
            if obj.usingFTGLTextRenderer
                for p=1:length(cache)
                    [~,~,~,cache(p)] = DrawFormattedText(cache(p),'cacheOnly',true,'sx',sx,'sy',sy,'xalign','center','yalign','center'); % FANI was DrawFormattedText2
                end
            else
                % offset the text to sx,sy (assumes it was centered on 0,0,
                % which is ok for current code)
...
%                 end
                % Offset the text to sx, sy (assumes it was centered on 0,0,
                % which is ok for current code)
                for p = 1:length(cache) % FANI
                    cache(p).px    = cache(p).px - previousOff(1) + sx;
                    cache(p).py    = cache(p).py - previousOff(2) + sy;
                    cache(p).bbox  = OffsetRect(cache(p).bbox, -previousOff(1) + sx, -previousOff(2) + sy);
                end           
            end
        end
...
%         end
        ...
        function drawFixPoints(obj,wpnt,pos,fixBackSize,fixFrontSize,fixBackColor,fixFrontColor)
            ...
            sz = [fixBackSize*2 fixFrontSize*2]; % FANI
        ...
        function [cal,selection] = showCalValResult(obj,wpnt,cal,selection)
            ...
            if ~isempty(butRectsBase)
                ...
                totWidth    = sum(buttonWidths)+(length(buttonWidths)-1)*buttonOff; 
                xpos        = [zeros(size(buttonWidths)).'; buttonWidths.']+[0 ones(1,length(buttonWidths)-1); zeros(1,length(buttonWidths))]*buttonOff;
                xpos        = cumsum(xpos(:))-totWidth/2+obj.scrInfo.resolution{end}(1)/2; % FANI TODO
                butRects(:,[1 3]) = [xpos(1:2:end) xpos(2:2:end)];
                butRects(:,2)     = yposBase-butRectsBase(:,4)+butRectsBase(:,2);
                butRects(:,4)     = yposBase;
            ...
            while ~qDoneWithAdvancedCalib
                ...
                while true
                    ...
                    if qShowHead
                        ...
                        if ~isempty(str)
                            DrawFormattedText(str,'win',wpnt(2),'sx','center','xalign','center','xlayout','center','sy',.03*RectHeight(headORect),'yalign','top','baseColor',obj.settings.UI.advcal.instruct.color,'vSpacing',obj.settings.UI.advcal.instruct.vSpacing,'winRect',headORect);
                        end % FANI was DrawFormattedText
                        if qShowHeadToAll
