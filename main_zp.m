...
%% Load custom path for testing
 data_struct = load('C:\Users\idm\Desktop\Semester4\Internship\Matlab\Data\test\rand_trials.mat');
 data = data_struct.rand_trials;
 folder_name = 'C:\Users\idm\Desktop\Semester4\Internship\Matlab\Data\test';
...
% for i = 1:size(data, 1)
for i = 1:4
%     % Get eye data, since start/last getting eye data
%     samp = eye_tracker.buffer.consumeN('gaze');
%     if ~isempty(samp)
%         % Use eye data here to control when trial starts/look at fixation
...
%     end
    ...
    samp = eye_tracker.buffer.consumeN('gaze');
    trial_results.samp(i) = samp; % add eye data to output
    %eyetracking_file = [subfolder_name, '\eyetracking_results_' , datestr(now, 'yyyymmdd_HHMM'), '.mat'];
    %save(eyetracking_file, 'samp');
