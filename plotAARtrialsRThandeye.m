...
plotADHDnonADHDVariance('Median Abandon Return Trial Rate Comparison',...
...
% todo make sure aar_percentages_adhd is mean instead of median too here
% aar_means_adhd      = mean(aar_percentages_adhd, 1, 'omitnan'); % Mean per condition
% aar_means_nonadhd   = mean(aar_percentages_nonadhd, 1, 'omitnan');
% plotADHDnonADHDVariance('Mean Abandon Return Trial Rate Comparison',...
%     aar_percentages_adhd, aar_means_adhd, aar_sem_adhd, 'ADHD', 'northeast', ...
%     aar_percentages_nonadhd, aar_means_nonadhd, aar_sem_nonadhd, 'nonADHD', 'northeast', ...
%     ids, condition_labels, '% of trials', group_labels, unique_conditions, color_map, color_map_individual, ...
%     fullfile(comparison_results_folder, '09_aar_trials_percentage_allinone_mean.png'));
...
se_adhd_eye         = nanstd(adhd_avg_eye, 0, 1) ./ sqrt(size(adhd_avg_eye, 1));
se_nonadhd_hand     = nanstd(nonadhd_avg_hand, 0, 1) ./ sqrt(size(nonadhd_avg_hand, 1));
se_nonadhd_eye      = nanstd(nonadhd_avg_eye, 0, 1) ./ sqrt(size(nonadhd_avg_eye, 1));
% non aar trials
...
se_adhd_n_hand      = nanstd(adhd_avg_n_hand, 0, 1) ./ sqrt(size(adhd_avg_n_hand, 1));
se_adhd_n_eye       = nanstd(adhd_avg_n_eye, 0, 1) ./ sqrt(size(adhd_avg_n_eye, 1));
se_nonadhd_n_hand   = nanstd(nonadhd_avg_n_hand, 0, 1) ./ sqrt(size(nonadhd_avg_n_hand, 1));
se_nonadhd_n_eye    = nanstd(nonadhd_avg_n_eye, 0, 1) ./ sqrt(size(nonadhd_avg_n_eye, 1));

% hand
plotADHDnonADHDVariance('Median RT of Button Press in Arrive Abandon Return Trials',...
    ...
    fullfile(comparison_results_folder, '09_aar_trials_rt_hand_allinone_median.png'));
plotADHDnonADHDVariance('Median RT of Button Press in non-AAR-Trials',...
    ...
    ids, condition_labels, 'Median RT (s)', group_labels, unique_conditions, color_map, color_map_individual, ...
    fullfile(comparison_results_folder, '09_nonaar_trials_rt_hand_allinone_median.png'));
% eye
plotADHDnonADHDVariance('Median Gaze Arrival at Target in Arrive Abandon Return Trials',...
    ...
    ids, condition_labels, 'Median RT (s)', group_labels, unique_conditions, color_map, color_map_individual, ...
    fullfile(comparison_results_folder, '09_aar_trials_rt_hand_allinone_median.png'));
plotADHDnonADHDVariance('Median Gaze Arrival at Target in non-AAR-Trials',...
...
se_nonadhd_n_diff      = nanstd(nonadhd_avg_n_diff, 0, 1) ./ sqrt(size(nonadhd_avg_n_diff, 1));
median_adhd_n_diff  = nanmedian(adhd_avg_n_diff, 1);
se_adhd_n_diff      = nanstd(adhd_avg_n_diff, 0, 1) ./ sqrt(size(adhd_avg_n_diff, 1));
plotADHDnonADHDVariance('Difference in Gaze Arrival at Target in Arrive Abandon Return Trials',...
    ...
    fullfile(comparison_results_folder, '09_aar_trials_rt_hand_diff_allinone_median.png'));
median_nonadhd_diff  = nanmedian(nonadhd_avg_diff, 1);
se_nonadhd_diff      = nanstd(nonadhd_avg_diff, 0, 1) ./ sqrt(size(nonadhd_avg_diff, 1));
median_adhd_diff  = nanmedian(adhd_avg_diff, 1);
se_adhd_diff      = nanstd(adhd_avg_diff, 0, 1) ./ sqrt(size(adhd_avg_diff, 1));
plotADHDnonADHDVariance('Difference in Gaze Arrival at Target in non-AAR-Trials',...
    ...
    ids, condition_labels, 'Difference in median RT (s)', group_labels, unique_conditions, color_map, color_map_individual, ...
    fullfile(comparison_results_folder, '09_nonaar_trials_rt_hand__diff_allinone_median.png'));
