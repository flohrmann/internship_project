function [nx, ny, textbounds, wordbounds] = DrawFormattedText(win, tstring, sx, sy, color, wrapat, flipHorizontal, flipVertical, vSpacing, righttoleft, winRect)
...
if isempty(ptb_drawformattedtext_oversize) ptb_drawformattedtext_oversize=1; end
if (ptb_drawformattedtext_oversize ~= 1) TS=Screen('TextSize',win); Screen('TextSize',win,round(TS/ptb_drawformattedtext_oversize)); end
    ...
    if ~isempty(curstring) && noclip
        ...
        if flipHorizontal || flipVertical
            ...
            if nargout >= 4 || padpergapneeded > 0
                ...
                if (ptb_drawformattedtext_oversize ~= 1) [nx ny] = Screen('DrawText',win,curstring, xp, yp, color, [], [],righttoleft);
                else [nx ny] = Screen('DrawText', win, curstring, xp, yp, color, [], yPosIsBaseline, righttoleft); end
            end
        end
    else
